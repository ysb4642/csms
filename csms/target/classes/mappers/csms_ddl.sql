CREATE DATABASE CSMS;

USE CSMS;

CREATE TABLE `ADMIN`(
    ADMIN_ID 	VARCHAR(20) PRIMARY KEY,
	PASSWD 		VARCHAR(100),
	JOIN_DT 	DATETIME
);

INSERT INTO ADMIN VALUES ('csmsadmin' , 'P@ssw0rd' , NOW());


CREATE TABLE MEMBER(
    MEMBER_ID 		VARCHAR(20) PRIMARY KEY,
    PASSWD 			VARCHAR(1000),
	MEMBER_NM 		VARCHAR(50),
    DATE_BIRTH 		VARCHAR(20),
	GENDER 			VARCHAR(10),
	HP 				VARCHAR(20),
	SMSSTS_YN 		VARCHAR(20),
	EMAIL 			VARCHAR(40),
	EMAILSTS_YN 	VARCHAR(20),
	POSTAL_CODE 	VARCHAR(20),
	ROAD_ADDRESS 	VARCHAR(500),
	PARCEL_ADDRESS 	VARCHAR(500),
	REST_ADDRESS 	VARCHAR(500),
	POINT			INT(11),
	JOIN_DT 		DATETIME
);

CREATE TABLE GOODS(
    GOODS_CD 		  INT(11) AUTO_INCREMENT PRIMARY KEY,
    GOODS_NM 		  VARCHAR(500) DEFAULT NULL,
	PRICE 			  INT(11) DEFAULT NULL,
    TASTE			  VARCHAR(50) DEFAULT NULL,
    EXPIRY_DATE		  VARCHAR(50) DEFAULT NULL,
    CAPACITY		  VARCHAR(50) DEFAULT NULL,
	DISCOUNT_RATE 	  INT(11) DEFAULT NULL,
	STOCK			  INT(11) DEFAULT NULL,
	BRAND 			  VARCHAR(50) DEFAULT NULL,
	SORT 			  VARCHAR(50) DEFAULT NULL,
	`POINT` 		  INT(11) DEFAULT NULL,
	DELIVERY_PRICE 	  INT(11) DEFAULT NULL,
	`EXPLAIN` 		  VARCHAR(2000) DEFAULT NULL,
	GOODS_FILE_NAME	  VARCHAR(2000) DEFAULT NULL,
	ENROLL_DT 		  DATETIME
);

 CREATE TABLE `ORDER`(
	ORDER_CD 			INT(11) AUTO_INCREMENT PRIMARY KEY,
	MEMBER_ID 			VARCHAR(20),
	GOODS_CD 			INT(11),
 	ORDER_GOODS_QTY 	INT(11),
 	PAYMENT_AMT			INT(11),
	ORDERER_NM 			VARCHAR(50),
	ORDERER_HP 			VARCHAR(50),
 	RECEIVER_NM 		VARCHAR(50),
 	RECEIVER_HP 		VARCHAR(50),
 	POSTAL_CODE 		VARCHAR(20),
 	ROAD_ADDRESS 		VARCHAR(500),
 	PARCEL_ADDRESS 		VARCHAR(500),
 	REST_ADDRESS 		VARCHAR(500),
	DELIVERY_METHOD 	VARCHAR(40),
	DELIVERY_MESSAGE 	VARCHAR(300),
	DELIVERY_STATUS 	VARCHAR(100),
 	PAY_METHOD 			VARCHAR(200),
 	PAY_ORDERER_HP 		VARCHAR(50), 
 	CARD_COMPANY_NM 	VARCHAR(50),
 	CARD_PAY_MONTH 		VARCHAR(20),
	PAY_ORDER_TIME 		DATETIME
 );

 CREATE TABLE CART (
 	CART_CD   	   INT(11) AUTO_INCREMENT PRIMARY KEY,
 	MEMBER_ID 	   VARCHAR(20),
 	GOODS_CD  	   INT(11),
 	CART_GOODS_QTY INT(11),
 	ENROLL_DT 	   DATETIME
 );

 CREATE TABLE CONTACT (
 	CONTACT_CD  INT(11) AUTO_INCREMENT PRIMARY KEY,
 	EMAIL		VARCHAR(50),
 	SUBJECT		VARCHAR(300),
 	CONTENT		VARCHAR(2000),
 	REG_DT  	DATETIME
 );
 
 CREATE TABLE REVIEW(
    REVIEW_CD 	INT(11) AUTO_INCREMENT PRIMARY KEY,
    GOODS_CD	INT(11),
	MEMBER_ID 	VARCHAR(20),
    RATING		INT(11),
	CONTENT 	VARCHAR(2000),
    ENROLL_DT	DATETIME
);